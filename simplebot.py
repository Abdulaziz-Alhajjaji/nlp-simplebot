...
client = OpenAI()  # Fixed: removed extra parenthesis
...
def send(event=None):  # Made event optional with default None
...
sendButton = Button(
    ...
    fg='#ffffff',
    command=send)  # This works now because event is optional

baseWindow.bind('<Return>', send)  # Keep only this binding for Enter key
